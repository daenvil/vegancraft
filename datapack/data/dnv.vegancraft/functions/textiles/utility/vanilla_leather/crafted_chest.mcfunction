# Called by dnv.vegancraft:textiles/utility/player_tick_real when player crafts leather chestplate

scoreboard players set @s dnv.craf_lchest 0
...
execute if score @s dnv.vleathers_d = @s dnv.leathers_d run scoreboard players remove @s dnv.vleathers 8
...
# Check if only 7 vegan leathers were spent:
...
# Update total counts
scoreboard players remove @s dnv.leathers 8